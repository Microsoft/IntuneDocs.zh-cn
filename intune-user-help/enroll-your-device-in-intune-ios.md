...
4. 此时会显示 "**选择设备和注册类型**" 屏幕并提示输入设备类型。  
    * 如果已从组织收到设备，请点击 " **（组织）拥有此设备**"。 然后，在本文中跳过[保护整个设备](###secure-entire-device)以完成安装。  
    * 如果使用的是从家里引入的个人设备，请点击 "**我拥有此设备**"。 然后继续执行下一步。  

    如果看不到此屏幕，请跳到 "[保护整个设备](enroll-your-device-in-intune-ios.md#secure-entire-device)以完成设置"。  
    
    ![公司门户 "选择设备和注册类型" 屏幕的示例屏幕截图，请选择设备类型选项。](./media/ios-device-type-1909.PNG)  


5. 选择在设备注册后如何保护其上的数据。  
    * 点击 "**保护整个设备**"，保护设备上的所有应用和数据。 然后，请执行[保护整个设备](enroll-your-device-in-intune-ios.md#secure-entire-device)以完成设置。
    * 请点击**安全的与工作相关的应用和数据，仅**保护你使用工作帐户访问的应用和数据。 然后，请[使用安全的与工作相关的应用程序和数据](enroll-your-device-in-intune-ios.md#secure-work-related-apps-and-data)。  

    ![公司门户 "选择设备和注册类型" 屏幕的示例屏幕截图，注册类型选项。](./media/ios-enrollment-type-1909.PNG)  


### <a name="secure-entire-device"></a>保护整个设备  

1. 在 "**设备管理" 和 "隐私**" 屏幕上，通读你的组织可查看的设备信息的列表。 然后点击“继续”  。  
...
2. Safari 在设备上打开公司门户网站。 当系统提示下载配置文件时，请点击“允许”  。 如果你的设备运行的是：  
    ...
    * iOS 12.1 及更早版本：下载完成后，会自动重定向到 "设置" 应用。 跳到步骤 4。  
 
    如果不小心点击“忽略”，请刷新页面  。 系统会提示打开公司门户应用。 完成后，再次点击 "**下载**"。

  > [!NOTE]
  > 必须在下载后的 8 分钟内按照后续步骤中的说明安装管理配置文件。 如果不这样做，配置文件将被删除，必须重新开始注册。  

3. 当系统提示打开公司门户时，点击 "**打开**"。 通读**如何安装管理配置文件**屏幕上的信息。  

4. 请在 "设置" 应用中，点击 "**注册 < 组织名称 >** 或**下载的配置文件**。  

    !["设置" 应用的示例屏幕截图，"按组织注册" 选项。](./media/enroll-in-organization-ios-1909.PNG)  

   如果这两个选项均未出现，请参阅 "设备管理"> **管理配置文件**& 的**常规** > **配置文件**。 如果仍没有看到管理配置文件，则可能需要再次下载。  
...
1. 此时将显示 "**下载 Microsoft Authenticator** " 屏幕（如果已有身份验证器，则不会看到此屏幕，请跳到步骤2）。  
    1. 点击 "**从应用商店下载**"。
    2. 当应用商店打开时，安装该应用。 
    3. 返回公司门户，然后点击 "**继续**"。    
    
   安装 Microsoft Authenticator 后，不需要对应用执行任何其他操作。 只需在设备上显示。 

   ![公司门户 "下载 Microsoft Authenticator" 屏幕的示例屏幕截图。](./media/download-ms-authenticator-1909.PNG)  

2. 在 "**设备管理" 和 "隐私**" 屏幕上，通读你的组织可查看的设备信息的列表。 然后点击“继续”  。  
...
3. Safari 在设备上打开公司门户网站。 当系统提示下载配置文件时，请点击“允许”  。 如果你的设备运行的是：  
    ...
    * iOS 12.1 及更早版本：下载完成后，会自动重定向到 "设置" 应用。 跳到步骤 5。  
...
4. 当系统提示打开公司门户时，点击 "**打开**"。 通读**如何安装管理配置文件**屏幕上的信息。 

5. 请在 "设置" 应用中，点击 "**注册 < 组织名称 >** 或**下载的配置文件**。  
...
6. 在 "**用户注册**" 屏幕上，点击 "**注册我的 iPhone"** 。  
    ...
    !["设置" 应用的示例屏幕截图，突出显示 "注册" 按钮。](./media/user-enrollment-information-1909.PNG)  

7. 输入设备密码。 然后点击“安装”  。  

8. 在**登录**屏幕上，输入托管 Apple ID 的密码。 在大多数情况下，这些凭据与你用于登录到工作帐户或学校帐户的凭据相同，除非你的组织提供了不同的凭据集。 
9. 点击 **"登录"** 。  
10. 安装配置文件后，屏幕上会显示一条成功消息。 若要验证配置文件是否已安装，请 **& 设备管理**" 设置中转到配置文件。 应该会在“移动设备管理”下看到此配置文件 **** 。  
