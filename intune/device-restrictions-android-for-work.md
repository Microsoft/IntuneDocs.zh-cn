...
- **多应用**：用户可访问设备上一组有限的应用。 设备启动时，仅添加的应用启动。 还可以添加一些用户可以打开的 Web 链接。 应用策略后，用户会在主屏幕上看到允许应用的图标。
  ...
  - **Wi-Fi 配置**：选择“启用”，允许最终用户将设备连接到不同的 WiFi 网络  。 启用此功能还会开启设备的位置。 “未配置”（默认）阻止用户在位于托管主屏幕（锁定任务模式）上时连接到 WiFi 网络  。

    有关详细信息，请参阅 [lock task mode](https://developer.android.com/work/dpc/dedicated-devices/lock-task-mode)（锁定任务模式）（打开 Android 的网站）。

  - **蓝牙配置**：选择“启用”以允许启用设备蓝牙且允许最终用户通过蓝牙配对设备  。 启用此功能还会开启设备的位置。 选择“未配置”（默认）会阻止用户在托管主屏幕（锁定任务模式）下配置蓝牙和配对设备  。 
...
- **允许访问 Google Play 商店中的所有应用**：将其设置为“允许”时，用户可以访问 Google Play 商店中的所有应用  。 他们无法访问管理员在[客户端应用](apps-add-android-for-work.md)中阻止的应用。 “未配置”强制用户仅能访问管理员对其显示的 Google Play 商店应用或[客户端应用](apps-add-android-for-work.md)中所需的应用  。
