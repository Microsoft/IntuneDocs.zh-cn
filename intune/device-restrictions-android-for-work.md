...
description: 在 Android Enterprise 或 Android for Work 设备上，限制设备上的设置，包括复制并粘贴、显示通知、应用权限、数据共享、密码长度、登录失败、使用指纹解锁、重复使用密码以及启用工作联系人蓝牙共享。 将设备配置为运行一个应用或多个应用的专用的设备网亭。
keywords: ''
...
- **跳过首次使用提示**：选择“启用”隐藏或跳过来自应用的建议，这些建议用于在应用启动时逐步提供教程或阅读任何介绍性提示。 设置为“未配置”时使用操作系统默认设置，该设置可在应用启动时显示这些建议。
...
### <a name="dedicated-device-settings"></a>专用的设备设置

使用这些设置专用设备上配置的网亭样式体验。 可以将设备配置为运行一个应用或运行多个应用。 设备设置为展台模式时，只能使用你添加的应用。 这些设置适用于专用的 Android 企业设备。 它们不会应用到完全托管的 Android 企业设备。

**展台模式**： 是否设备运行一个应用或运行多个应用选择。
...
- **单应用**：用户只能访问设备上的单个应用。 设备启动时，仅特定应用启动。 限制用户打开新应用或更改正在运行的应用。
  ...
  **步骤**
  1. 选中“选择托管应用”，并从列表中选择托管的 Google Play 商店应用。 
      ...
      如果未列出任何应用，则可以将[一些 Android 应用](apps-add-android-for-work.md)添加到设备。 请确保[将应用分配给为专用设备创建的设备组](apps-deploy.md)。
...
- **多应用**：用户可访问设备上一组有限的应用。 设备启动时，仅添加的应用启动。 还可以添加一些用户可以打开的 Web 链接。 应用策略后，用户会在主屏幕上看到允许应用的图标。
  ...
  - 选择“添加”并从列表中选择应用。
    ...
    如果未列出“托管主屏幕”应用，则[从 Google Play 添加该应用](https://play.google.com/work/apps/details?id=com.microsoft.launcher.enterprise)。 请确保将[应用分配](apps-deploy.md)给为专用设备创建的设备组。

    还可以将其他由组织创建的 [Android 应用](apps-add-android-for-work.md)和 [Web 应用](web-app.md)添加到设备。 请确保[将应用分配给为专用设备创建的设备组](apps-deploy.md)。

  - **虚拟主页按钮**：选择“启用”可在专用设备上显示主页按钮。 选中后，会将用户返回至设备的主屏幕，以便用户可以轻松地在应用之间进行切换。 在某些 Android 设备上，用户可能需要向上轻扫屏幕才能显示主页按钮。 “禁用”则不会显示主页按钮，因此，用户必须使用后退按钮才能在应用之间进行切换。
  - **退出展台模式**：选择“启用”可允许管理员临时暂停展台模式以更新设备。 若要使用此功能，管理员： 
...
- **禁用锁屏功能**： 启用锁屏时在设备上，选择要禁用的功能。 例如，勾选“安全照相机”会禁用设备上的照相机功能。 所有未勾选的功能均在设备上保持启用状态。
