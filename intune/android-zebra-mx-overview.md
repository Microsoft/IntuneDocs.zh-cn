...
Intune 提供丰富的功能，包括管理应用和配置设备设置。 这些内置功能和设置用于管理 Zebra Technologies 公司制造的 Android 设备，这些设备也称为“Zebra 设备”。

在 Android 设备上，使用“Mobility Extensions (MX)”配置文件来自定义或添加更多 Zebra 特定设置。
...
在设备上，转到 Google Play 商店，然后从 Microsoft 下载并安装 Intune 公司门户应用。 从 Google Play 安装后，公司门户应用会自动获取更新和修复程序。

如果无法使用 Google Play，请下载[适用于 Android 的 Microsoft Intune 公司门户](https://www.microsoft.com/download/details.aspx?id=49140)（打开另一个 Microsoft 网站），然后[进行旁加载](#sideload-the-company-portal-app)（本文有介绍）。 以这种方式进行安装，应用不会自动接收更新或修复程序。 应定期更新并手动修补应用。
...
[注册 Android 设备](android-enroll.md)列出了相关步骤。 如果有多个 Zebra 设备，建议使用[设备注册管理器 (DEM) 帐户](device-enrollment-manager-enroll.md)。 使用 DEM 帐户还会删除“从公司门户应用取消注册”选项，以便用户不会轻易取消注册设备。
...
在 StageNow 中创建配置文件时，在最后一步中，选择“导出到 MDM”。 此操作会生成一个 XML 文件。 请保存该文件。 在后面的步骤中需要使用该文件。
...
> - 用于[添加](apps-add.md)、[部署](apps-deploy.md)、更新和[监视](apps-monitor.md)应用等的应用管理功能。
...
> [!NOTE]
> 将配置文件部署到每个设备。 如果要在设备上部署多个 StageNow 配置文件，则导出 StageNow 配置文件，将设置合并到一个 XML 文件中，然后再添加到 Intune。 
> 
> 不建议在同一 XML 文件中包含两个可配置同一属性的设置。 这是为防止设备上的设置之间产生冲突。
...
下次设备检查配置更新时，MX 配置文件会部署到设备。 设备注册时，会与 Intune 进行同步，然后约每隔 8 小时同步一次。 还可以[在 Intune 中强制执行同步](device-sync.md)。 或者，在设备上，打开“公司门户应用” > “设置” > “同步”。 

> [!TIP]
> - 出于安全原因，保存后无法看到配置文件 XML 文本。 文本已加密，只能看到星号 (`****`)。 为了便于参考，建议在将 MX 配置文件添加到 Intune 之前保存其副本。
> 
> - 为了在将配置文件分配到 Zebra 设备后对其进行更新，请创建更新后的 StageNow XML 文件，编辑现有 Intune 配置文件，并添加新的 StageNow XML 文件。 此新文件将覆盖配置文件中之前的 StageNow 策略。
