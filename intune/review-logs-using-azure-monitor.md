...
Microsoft Intune 包含可提供环境信息的内置日志。 审核日志显示 Intune 中发生的各种事件或任务的详细信息。 运行日志（预览版）显示注册成功（或失败）的用户和设备的详细信息。
...
3. 输入以下属性：
    ...
    - “日志” > “运行日志”：运行日志（预览版）显示用户和设备是否已成功注册 Intune。 选中此选项可以将注册日志发送到存储帐户、事件中心或 Log Analytics。
