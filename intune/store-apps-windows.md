...
5. 选择列表上方的“添加”。
6. 从列表的可用“商店应用”类型中选择“Windows”。
7. 选择“应用信息”，配置应用信息。
8. 在“应用信息”边栏选项卡中，配置以下信息，根据所选应用，此窗格中的一些值可能已自动填入：
    ...
    - **类别**（可选）- 选择一个或多个应用类别或你创建的类别。 添加此信息可让用户在浏览公司门户时更轻松地查找应用。
    ...
    - **上传图标** - 上传将与应用关联的图标。 用户浏览公司门户时，此图标随应用一同显示。
1. 输入所有需要的应用信息后，单击“确定”。
2. 在“添加应用”边栏选项卡中，单击“添加”。
...
5. 将公司门户应用上传到 Intune 之前，创建一个文件夹（例如，C:\Company Portal），其中包含按以下方式构建的包：
  ...
  - 将九个依赖项包置于 Dependencies 文件夹中。 如果依赖项未按此格式放置，则 Intune 无法在包上传期间将其识别并上传，从而导致上传失败并出现以下错误： 
...
我们推荐的迁移途径是通过将分配操作设置为“卸载”，删除 Windows 8.1 公司门户应用的分配。 完成此设置后，就可使用上面任意选项分配 Windows 10 公司门户应用。  
