...
- [设置自动注册](../enrollment/quickstart-setup-auto-enrollment.md) - 将 Intune 设置为在特定用户登录到 Windows 10 设备时自动注册设备。
- [注册设备](../enrollment/quickstart-enroll-windows-device.md) - 使用 Intune 用户角色并将设备注册到 Intune。 然后，返回 Intune 并确认设备注册成功。
- [创建设备符合性策略](../protect/quickstart-set-password-length-android.md) - 创建设备符合性策略并为策略分配组。
- [向不符合要求的设备发送通知](../protect/quickstart-send-notification.md) - 通过创建和分配符合性策略，向使用不符合要求的设备的员工发送电子邮件通知。
- [添加和分配应用](../apps/quickstart-add-assign-app.md) - 添加客户端应用并将其分配给公司的员工。
- [创建和分配应用保护策略](../apps/quickstart-create-assign-app-policy.md) - 创建应用保护策略并将其分配给最终用户设备上的客户端应用。
- [创建和分配自定义角色](create-custom-role.md) - 创建并分配具有安全操作部门的特定权限的自定义角色。 
- [创建用于 iOS 的电子邮件设备配置文件](../configuration/quickstart-email-profile.md) - 创建用于 iOS 设备的电子邮件设备配置文件。
...
![Microsoft Intune 的高级体系结构图表](./media/get-started-evaluation/intunearchitecture.svg)

## <a name="next-steps"></a>后续步骤

[Azure 使用入门](tutorial-walkthrough-intune-portal.md) - 了解 Azure 门户的结构，以及如何更改看到的页面。

## <a name="learn-more"></a>了解详细信息

- [什么是 Intune？](what-is-intune.md)
- [什么是 Azure 门户？](what-is-intune.md)
- [设备和应用生命周期](device-lifecycle.md)
