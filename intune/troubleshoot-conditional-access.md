...
- 对于 iOS 设备：
  - 用户创建的现有电子邮件配置文件将阻止由 Intune 管理员创建的配置文件的部署。 这是一个常见问题，因为 iOS 用户通常将创建电子邮件配置文件，然后注册。 公司门户将通知用户由于他们的电子邮件配置文件是手动配置的，因此不合规，并提示用户删除该配置文件。 用户应删除其电子邮件配置文件，以便可部署 Intune 配置文件。 为防止此问题，请告知用户在未安装电子邮件配置文件时进行注册，并允许 Intune 部署配置文件。
  - iOS 设备可能在检查合规性状态过程中受阻，进而阻止用户初始化其他签入。 重启公司门户可能可以修复此问题，且合规性状态将反映 Intune 中的设备状态。 通过设备同步收集所有的数据后，合规性检查将快速进行，平均速度不超过半秒。

    通常，设备保持此状态的原因是其在连接到服务时出现问题或同步花费的时间较长。  设备重启且已验证设备上的 SSP 为最新后，如果此问题继续存在于不同网络配置（移动电话网络、Wi-Fi、VPN）中，请按照[获取对 Microsoft Intune 的支持](get-support.md)中的说明，与 Microsoft 支持人员取得联系。
...
- 对于 Android 设备：
   - 某些 Android 设备看似已加密，但公司门户应用还是会将这些设备识别为未加密。 
       ...
       -   某些设备制造商使用默认 PIN，而不是用户设置的机密 PIN 来加密其设备。 Intune 会将使用默认 PIN 的加密识别为不安全，因为这种加密方式会让设备上的数据受到威胁，使数据可能泄露给对设备具有物理访问权限的恶意用户。 如果存在此问题，建议使用[应用保护策略](app-protection-policies.md)。
...
- 是否存在可隔离或阻止设备的默认 Exchange 规则？ 如果默认的规则阻止或隔离设备，设备将无法从 Exchange Connector 中收到激活电子邮件。 这是由于设计而导致的。
...
- 查看 Exchange Connector 日志以获取发送电子邮件活动并检查错误。 要搜索的命令示例为从通知帐户到用户邮箱的 SendEmail。
...
若要查看 Exchange Connector 日志，请使用[服务跟踪查看器工具](https://docs.microsoft.com/en-us/dotnet/framework/wcf/service-trace-viewer-tool-svctraceviewer-exe)。 此工具需要下载 Windows Server SDK。
