...
本主题旨在帮助 IT 管理员为通过 [Apple School Manager](https://school.apple.com/) 计划购买的设备启用 iOS 设备注册。 Microsoft Intune 可以通过“无线方式”部署注册配置文件，该配置文件会注册 Apple School Manager 设备来对其进行管理。 管理员始终不需要接触每个受管理设备。 注册配置文件包含注册时应用于设备的管理设置，包括“设置助理”选项。
...
>[!NOTE]
>Apple School Manager 注册不能与 [Apple DEP](device-enrollment-program-enroll-ios.md) 或[设备注册管理器](device-enrollment-manager-enroll.md)一起使用。
...
**步骤 2：下载令牌并分配设备。**<br>
选择“通过 Apple School Manager 创建令牌”，然后通过公司 Apple ID 登录。 可使用此 Apple ID 续订 Apple School Manager 令牌。
...
5. 对于“用户关联”，请选择具有此配置文件的设备是否通过用户关联进行注册。
 ...
 >[!NOTE]
 >在具有用户关联的 Apple School Manager 设备上注册期间，多重身份验证 (MFA) 不起作用。 注册之后，MFA 在这些设备上会正常运行。

  Apple School Manager 的“Shared iPad”模式要求用户不通过用户关联进行注册。

 >[!NOTE]
    >具有用户关联的 Apple School Manager 要求启用 [WS-Trust 1.3 用户名/混合终结点](https://technet.microsoft.com/library/adfs2-help-endpoints)以请求用户令牌。 [详细了解 WS-Trust 1.3](https://technet.microsoft.com/itpro/powershell/windows/adfs/get-adfsendpoint)。
...
至此，已将管理 Apple School Manager 设备的权限分配给 Intune，接下来可以通过 Apple 服务同步 Intune，以便在 Intune 门户中查看托管设备。
