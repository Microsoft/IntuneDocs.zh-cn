...
可以将[传递优化设置](delivery-optimization-settings.md)为设备配置文件的一部分。
...
> [!NOTE]
> 你可以使用 Microsoft Intune [作为具有移动设备管理 (MDM) 的移动设备](windows-enroll.md)或具有 Intune 软件客户端的计算机来管理 Windows 电脑。 Microsoft 建议客户尽可能[使用 MDM 管理解决方案](windows-enroll.md)。 当以这种方式进行管理时，不支持 BranchCache。 有关详细信息，请参阅[对比作为计算机或移动设备管理 Windows 电脑](pc-management-comparison.md)。
