...
8. 对于特定应用，请选择“分配类型”：
   ...
   - 注册与否都可用：可将此应用分配到未将其设备注册到 Intune 的用户组。 来自托管的 Google Play 和 macOS LOB 应用的应用不支持此选项。 
   - 必需：应用安装在所选组中的设备上。
   - 卸载：已从所选组中设备上卸载应用。
...
|可用用户|设备卸载|同时存在，可用的网关解析。<br><br>应用将显示在公司门户中。<br><br>如果应用已安装（作为前一意图的必需应用），将卸载应用。<br><br>如果用户选择“从公司门户安装”，则将安装应用，忽略卸载意图。|
|不可用的用户|必需设备|必需|
|不可用的用户|设备卸载|卸载|
...
|必需和可用的用户|设备卸载|同时存在，必需的网关解析（必需和可用）
|不可用的用户|不可用的设备|不可用|
|可用用户|不可用的设备|可用|
|必需用户|不可用的设备|必需|
