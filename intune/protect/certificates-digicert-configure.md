...
3. 选择“下载证书连接器软件”  。 将软件保存到可以从服务器上进行访问的位置，将在该服务器上安装该软件。  
...
2. 在 Intune 门户中创建受信任的证书配置文件：
   ...
   a. 登录到 [Intune](https://go.microsoft.com/fwlink/?linkid=2090973)。

   b. 选择“设备配置” > “管理” > “配置文件” > “创建配置文件”     。

   c. 为受信任的证书配置文件输入“名称”和“描述”详细信息   。

   d. 从“平台”  下拉列表中，为此受信任证书选择设备平台。

   e. 从“配置文件类型”下拉列表中，选择“受信任的证书”   。

   f. 浏览到上一步从 DigiCert CA 获取的受信任的根 CA .cer 文件，然后选择“确定”  。

   g. 从以下位置选择受信任证书的目标存储区（仅适用于 Windows 8.1 和 Windows 10 设备）：    
      - **计算机证书存储区 - 根**  
      - **计算机证书存储区 - 中间**  
      - **用户证书存储区 - 中间** 

   h. 完成后，选择“确定”，返回“创建配置文件”窗格，然后选择“创建”    。  
...
2. 转到“设备配置”   >  “配置文件”  ，然后选择“创建配置文件”  。

3. 为 PKCS 证书配置文件输入“名称”和“描述”详细信息   。  

4. 从“平台”  下拉列表中，选择支持的设备平台。

5. 从“配置文件类型”下拉列表中，选择“PKCS 证书”   。
 
6. 在“PKCS 证书”  窗格中，使用下表中的值配置参数。 需要这些值才能通过 Intune 证书连接器从 DigiCert CA 颁发 PKCS 证书。 
...
7. 完成配置文件的配置以满足你的业务需求，然后选择“确定”  以保存配置文件。 

8. 选择“分配”  ，并配置将接收此配置文件的相应组。 至少一个用户或设备必须成为已分配组的一部分。
...
| 问题/错误消息 | 解决步骤 |
| --- | --- |
| 无法使用 Intune 租户管理员帐户登录到 NDES 连接器 UI。 | Intune 管理门户中未启用本地证书连接器时，可能会发生这种情况。 若要解决此问题，请使用以下任一过程： <br><br> 从 Silverlight UI： <br> 1.登录到 [Intune 管理门户](https://admin.manage.microsoft.com)。 <br> 2.选择“管理员”  。 <br> 3.选择“移动设备管理”   > “证书连接器”  。 <br> 4.选择“配置本地证书连接器”  。 <br> 5.选中“启用证书连接器”  复选框。 <br> 6.选择“确定”  。 <br><br> 从 Azure 门户 UI： <br> 1.登录到 [Azure 门户](https://portal.azure.com)。 <br> 2.转到 Microsoft Intune。 <br> 3.选择“设备配置”   > “证书颁发机构”  。 <br> 4.选择“启用”  。 <br><br> 在从 Silverlight UI 或 Azure 门户完成前述步骤之后，尝试使用相同的 Intune 租户管理员帐户登录到 NDES 连接器 UI。 |
