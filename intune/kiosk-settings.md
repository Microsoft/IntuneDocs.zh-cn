...
description: 将 Windows 10（及更高版本）设备配置为单应用和多应用展台、自定义开始菜单、添加应用、显示任务栏和配置 Web 浏览器。 并在 Microsoft Intune 中将 Windows Holographic for Business 设备配置为多应用展台。
keywords: ''
...
ms.locfileid: "54203461"
---
# <a name="windows-10-and-later-device-settings-to-run-as-a-dedicated-kiosk-using-intune"></a>使用 Intune 将 Windows 10（及更高版本）设备设置作为专用展台运行的设置
...
本文列出并介绍了可以在 Windows 10 和更高版本设备上控制的不同设置。 作为移动设备管理 (MDM) 解决方案的一部分，请使用这些设置将 Windows 10 设备配置为在展台模式下运行。
...
还可以为 [Android](device-restrictions-android.md#kiosk) 和 [Android Enterprise](device-restrictions-android-for-work.md#kiosk-settings) 设备创建展台配置文件。
