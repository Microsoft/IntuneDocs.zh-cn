...
使用 Intune 的安全基线来帮助保护用户和设备的安全。 安全基线是预配置的 Windows 设置组，可帮助应用已知设置组以及相关安全团队建议的默认值。 在 Intune 中创建安全基线配置文件时，将创建“设备配置”  配置文件。
...
## <a name="security-baseline-versions-and-instances"></a>安全基线版本和实例
基线的新更新会不定期提供。 基线的每个新版本实例可以添加或删除设置或引入其他更改。 例如，当新版本的 Windows 10 中提供新 Windows 10 设置时，MDM 安全基线可能会收到包含最新设置的新版本实例。  

在 Intune 控制台中，可以查看可用的安全基线及其相关信息。 可用信息包括使用该基线类型的配置文件数量，可用基线类型的单独实例的数量，以及提供或发布最新实例的时间。  以下示例展示了频繁使用的 MDM 安全基线的磁贴：  

![基线磁贴](./media/security-baselines/baseline-tile.png)

若要查看所使用的基线版本的信息，请选择一个基线，然后选择“版本”  。 Intune 显示你的配置文件使用的版本的相关详细信息。 在“版本”窗格中，可以选择单个版本以进一步详细了解使用该版本的配置文件。 还可以选择两个不同的版本，然后选择“比较基线”  ，下载详细介绍其差异的 CSV 文件。  
...
安全基线配置文件支持[更改正在使用的基线版本](#change-the-baseline-instance-for-a-profile)。 这意味着当新版本推出时，无需创建新的基线配置文件即可充分利用它。 相反，当一切就绪后，可以选择基线配置文件，然后使用内置选项更改该配置文件的实例版本。  
...
## <a name="change-the-baseline-instance-for-a-profile"></a>更改配置文件的基线实例
基线配置文件支持更改配置文件使用的基线实例。 可以选择较低版本的实例，更常见的做法是选择相同基线的较新实例。  不能在两个不同的基线之间进行更改，例如将配置文件从使用 Defender ATP 的基线更改为使用 MDM 安全基线。 

在配置基线版本的更改时，可以选择下载列出了所涉及的两个基线版本之间的更改的 CSV 文件。 还可以选择保留原始基线版本中的所有自定义项，并将其应用于最新版本，或实现在所选的新基线版本中找到的所有默认值。 
...
### <a name="to-change-the-instance-for-a-baseline"></a>更改基线实例  
