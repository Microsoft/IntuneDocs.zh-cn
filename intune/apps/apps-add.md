...
Intune 还支持对需要安全访问本地数据的客户端应用（例如业务线应用服务器）启用访问权限。 提供此类型的访问通常需要将用于访问控制的 [Intune 托管证书](../protect/certificates-configure.md)与外围中的标准 VPN 网关或代理（例如 Azure Active Directory 应用程序代理）结合使用。 可以借助 Intune 的[应用包装工具和应用 SDK](../developer/apps-prepare-mobile-application-management.md) 将已访问的数据包含在业务线应用中，这样它就不能将公司数据传递给使用者应用或服务。
