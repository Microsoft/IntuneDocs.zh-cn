...
应用保护数据的保持期为 90 天。 在过去 90 天内签入到 MAM 服务的任何应用实例都将包含在“应用保护状态”报表中。 应用实例  是唯一的用户 + 应用 + 设备。 
...
可以通过选择“用户状态”  磁贴（基于设备操作系统平台）、“使用潜在有害应用的用户”  磁贴和“已标记用户”  磁贴转到摘要的详细视图。
...
>[!NOTE]
> “上次同步时间”列在控制台内“用户状态”报表和“应用保护策略”[可导出的 .csv 报表](https://docs.microsoft.com/intune/app-protection-policies-monitor#export-app-protection-activities-to-csv)中表示相同的值  。 差别在于两个报表中的值之间的同步存在较小延迟。 
>
...
在此处报告具有由“要求对应用进行威胁扫描”条件启动检查标记的设备的用户，威胁类别是“由 Google 报告”  。 如果此报表中列出了通过 Intune 部署的应用，请与应用开发人员联系，或删除要分配给用户的应用。 
...
- **验证结果**：数据按应用保护状态分组，可能是“失败”、“警告”或“成功”。
- **应用名称**：数据按实际应用名称分组。 同样，状态可能是“失败”、“警告”或“成功”。
