...
| 最终用户取消了应用安装。 (0xC7D14FB1) | 用户已显式卸载该应用。 如果用户取消了 Android OS 安装活动，则会返回此错误。 出现 OS 安装提示时，用户点击了“取消”按钮，或单击关闭了提示。 此错误仅在 DA 方案中返回。 对于 KNOX 方案，用户不会收到安装提示，此操作以无提示方式执行。 Intune 将提供可供用户单击以进行重试的通知。 如果该应用是可选应用，则可以忽略该通知。 但是，如果是必需应用，则不能忽略。 请让用户不要取消安装。 |
...
| 文件下载服务意外停止。 下次同步设备时将重试该策略。 (0xC7D1507C) | OS 已在下载进程完成前将其停止。 如果设备电量过低或下载耗时过长，则会出现此错误。 此错误仅在 DA 方案中返回。 对于 KNOX 方案，用户不会收到安装提示，此操作以无提示方式执行。 手动同步设备或等待 24 小时，然后检查状态。 |
| 未能卸载应用。 (0xc7d14fb8) | 此错误是一般的卸载故障。 操作系统未说明应用卸载失败的原因。 某些管理应用无法直接卸载。 如果卸载失败，请检查以确保应用可以手动卸载并收集公司门户日志。 |
| 用于升级的应用安装 APK 文件与设备上当前应用的签名不匹配。 (0xc7d14fb7) | Android OS 具有以下限制：要求升级版本的签名证书需与用于签署现有版本的证书完全相同。 如果开发人员无法使用相同的证书对新版本进行签名，则需要卸载现有应用并重新部署新应用，而不是升级现有应用。 |
...
| 无法安装应用安装 APK 文件，因为该文件未签名。 (0xc7d14fb6) | 默认情况下，Android OS 要求对应用进行签名。 请确保在部署之前对应用进行签名。 |

## <a name="ios-app-installation-errors"></a>iOS 应用安装错误
