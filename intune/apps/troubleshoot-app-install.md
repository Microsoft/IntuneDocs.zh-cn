...
本部分介绍设备管理员（DA）和 Samsung Knox 注册。 有关详细信息，请参阅[android 设备管理员注册](../enrollment/android-enroll-device-administrator.md)和[使用 Samsung 的 Knox Mobile 注册自动注册 Android 设备](../enrollment/android-samsung-knox-mobile-enroll.md)。 
...
| 最终用户取消了应用安装。 (0xC7D14FB1) | 用户已显式卸载该应用。 如果用户取消了 Android OS 安装活动，则会返回此错误。 出现 OS 安装提示时，用户点击了“取消”按钮，或单击关闭了提示。 此错误仅在 DA 方案中返回。 对于 KNOX 方案，用户不会收到安装提示，此操作以无提示方式执行。 Intune 将提供可供用户单击以进行重试的通知。 如果该应用是可选应用，则可以忽略该通知。 但是，如果是必需应用，则不能忽略。 要求用户不要取消安装。 |
| 文件下载进程意外停止。 (0xC7D15015) | OS 已在下载进程完成前将其停止。 如果设备电量过低或下载耗时过长，则会出现此错误。 此错误仅在 DA 方案中返回。 对于 KNOX 方案，用户不会收到安装提示，此操作以无提示方式执行。 Intune 将提供可供用户单击以进行重试的通知。 如果该应用是可选应用，则可以忽略该通知。 但是，如果是必需应用，则不能忽略。 确保设备具有可靠的网络连接。  |
| 文件下载服务意外停止。 下次同步设备时将重试该策略。 (0xC7D1507C) | OS 已在下载进程完成前将其停止。 如果设备电量过低或下载耗时过长，则会出现此错误。 此错误仅在 DA 方案中返回。 对于 KNOX 方案，用户不会收到安装提示，此操作以无提示方式执行。 手动同步设备或等待24小时，然后检查状态。 |
| 未能卸载应用。 (0xc7d14fb8) | 此错误是一般卸载失败。 操作系统未指定应用卸载失败的原因。 某些管理应用无法直接卸载。 如果卸载失败，请检查以确保应用可以手动卸载并收集公司门户日志。 |
| 用于升级的应用安装 APK 文件与设备上当前应用的签名不匹配。 (0xc7d14fb7) | Android OS 的限制要求升级版本的签名证书与用于签署现有版本的证书完全相同。 如果开发人员不能使用相同的证书对新版本进行签名，则需要卸载现有应用并重新部署新应用，而不是升级现有应用。 |
| 最终用户取消了应用安装。 (0xc7d14fb9) | 让用户接受 Intune 部署的应用，并在出现提示时安装应用。 |
| 安装期间重启了进程，因此取消了应用卸载。 (0xc7d14fbc) | 操作系统终止了应用安装进程，或者设备已重启。 如果再次发生此错误，请重试安装并收集公司门户日志。 |
| 无法安装应用安装 APK 文件，因为它未签名。 (0xc7d14fb6) | 默认情况下，Android OS 要求对应用进行签名。 请确保在部署之前对应用进行签名。 |
...
| 用户拒绝了安装应用的提议。 (0x87D13B62) | 初始应用安装过程中，用户单击了“取消”。 要求用户在下次接受安装请求。 |
...
| 许可证分配失败，Apple 错误 "没有剩余的 VPP 许可证" （0x87d13b7e） | 此行为是设计使然。 若要解决此问题，请购买更多的 VPP 许可证，或回收不再面向用户的许可证。 |
| 应用安装失败12024：原因未知。 (0x87d13b6e) | Apple 没有提供足够的信息来确定安装失败的原因。 没有要报告的内容。 |
| 所需的应用配置策略不存在，请确保策略以相同的组为目标。 (0x87d13b7f) | 应用需要应用配置，但不会针对任何应用配置。 管理员应确保应用的目标组也拥有目标为的组所需的应用配置。 |
| 设备 VPP 许可仅适用于 iOS 9.0 + 设备。 (0x87d13b69) | 升级受影响的 iOS 设备到 iOS 9.0 +。 |
| 此应用程序安装在设备上，但不受管理。 (0x87d13b8f) | 此错误仅发生在 LOB 应用程序中。 应用安装在 Intune 外。 若要解决此错误，请从设备中卸载该应用。 下一次设备同步发生时，设备应从 Intune 安装应用。 |
| 用户拒绝的应用管理（0x87d13b68） | 要求用户接受应用管理。 |
| 未知错误。 (0x87d1279d) | IOS 应用商店应用发生此错误，但错误情况未知。 |
| 最新版本的应用无法从早期版本更新。 (0x87D13B9D) | 如果在设备上安装和管理了应用但版本不正确，则会显示此错误消息。 这种情况包括：设备接收到更新应用的命令，但尚未安装和报告新版本。 在部署升级后，将报告设备的第一次签入此错误，并将发生，直到设备报告新版本已安装，或由于其他错误而失败。  |
