---
title: "将应用分配到 Android for Work 设备 | Microsoft Docs"
titleSuffix: Intune Azure preview
description: "Intune Azure 预览版：使用本主题可从 Google Play for Work 商店同步应用，然后将应用分配到 Android for Work 设备。"
keywords: 
author: robstackmsft
ms.author: robstack
manager: angrobe
...
此外，如果你创建了自己的业务线 (LOB) 应用，则可以分配它们。 若要执行该操作，你需要注册 Google 开发人员帐户，该帐户使你可以将应用发布到 Google Play 商店中的专用区域存储，然后将它们与 Intune 同步。
...
如果已从该商店批准了应用，但尚未在“移动应用”工作负荷的“获得许可的应用”节点中看到它，则可以强制立即同步，如下所示：

1. 登录到 Azure 门户中。
2. 选择“更多服务” > “监视 + 管理” > “Intune”。
3. 在“Intune”边栏选项卡上，选择“移动应用”。
4. 在“移动应用”工作负荷中，选择“设置” > “Android for Work”。
5. 在“Android for Work”边栏选项卡上，选择“立即同步”。
6. 该页还会显示上次同步的时间和状态。
...
Android for Work 需要你先在 Google 的托管 Play Web 控制台中批准应用，然后才能将应用同步到 Intune 并分配给用户。  由于 Android for Work 允许以无提示方式将这些应用自动推送到用户的设备，因此你必须代表所有用户接收应用的权限。  最终用户安装应用时将不会看到任何应用权限，因此你必须阅读并理解这些权限。
...
需定期访问托管的 Google Play 控制台，查看是否有新权限。 如果分配了应用，但发现设备上未安装此应用，请按照以下步骤检查是否有新权限：
