...
ms.locfileid: "52728763"
---
# <a name="windows-10-and-newer-device-restriction-settings-in-microsoft-intune"></a>Microsoft Intune 中的 Windows 10（及更高版本）设备限制设置

本文列举并介绍了可以为 Windows 10 设备配置的所有设备限制设置。 将这些设置添加到设备配置文件，然后使用 Microsoft Intune 将其分配或部署到设备中。
...
- **锁定屏幕图片 URL (仅限桌面版)**：输入将用作 Windows 锁定屏幕墙纸的图片（格式为 JPEG）的 URL。 用户无法更改此设置。
...
- **Microsoft 兼容性列表**：“阻止”可阻止 Microsoft Edge 中的 Microsoft 兼容性列表。 此 Microsoft 列表可帮助 Edge 正确显示具有已知兼容性问题的站点。 “未配置”则允许使用 Microsoft 兼容性列表。
