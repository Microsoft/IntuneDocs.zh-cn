...
-   **由 Intune 管理的设备：** 对于在 Intune 中注册的设备，自动允许在具有应用保护策略的应用与其他通过 Intune 部署的托管 iOS 应用之间进行数据传输。 要指定数据传输到其他应用的所需方式，请启用“允许应用向其他应用传送数据”设置并选择首选的共享级别。 要指定希望如何允许应用从其他应用接收数据，请启用“允许应用从其他应用接受数据”设置并选择首选的接收数据级别。 可使用“打开方式管理”功能控制在通过 Intune 部署的应用之间进行的数据传输。 有关如何接收和共享应用数据的详细信息，请参阅[数据重定位设置](app-protection-policy-settings-ios.md#data-relocation-settings)。   
