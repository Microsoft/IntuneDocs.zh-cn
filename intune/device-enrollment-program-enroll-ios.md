...
若要启用 DEP 注册，需同时使用 Intune 和 Apple DEP 门户。 还需要 DEP 设备的列表或采购订单号，从而可以将其分配到 Intune 以在 Apple 门户中进行管理。
...
选择“通过 Apple 设备注册计划创建令牌”以打开 Apple 的部署计划门户，并使用公司 Apple ID 登录。 可使用此 Apple ID 续订 DEP 令牌。
  ...
  ![Apple 证书工作区中用于下载公钥的“注册计划令牌”窗格的屏幕截图。](./media/enrollment-program-token-sign.png)
   1.  在 Apple 的[部署计划门户](https://deploy.apple.com)中，对“设备注册计划”选择“开始使用”。
   ![对“设备注册计划”单击“开始使用”时“注册计划”的屏幕截图。](./media/enrollment-program-token-started.png)
   2. 在“管理服务器”页上，选择“添加 MDM 服务器”。
   ...
   4.  “添加&lt;服务器名称&gt;”对话框随即打开，提示“上传公钥”。 选择“选择文件…” 以上传 .pem 文件，然后选择“下一步”。
   ...
   7. 在“选择操作”中，选择“分配到服务器”，选择为 Microsoft Intune 指定的 &lt;服务器名称&gt;，然后选择“确认”。 Apple 门户将指定的设备分配到 Intune 服务器以进行管理，然后显示“分配完成”。
...
4. 选择“设备管理设置”配置以下配置文件设置：![选择“受到监管”、“注册锁定”、“允许设置为拒绝所有配对”管理模式，Apple Configurator 证书针对新注册计划配置文件灰显的屏幕截图。](./media/enrollment-program-profile-mode.png)
...
5. 选择“设置助理设置”配置以下配置文件设置：![选择“配置设置”，其中有可用设置用于新注册计划配置文件的屏幕截图。](./media/enrollment-program-profile-settings.png)
...
必须先向 Intune 托管的设备分配注册计划配置文件，然后再注册设备。
...
现在可以将设备分配给用户。 具有用户关联的设备需要每个用户都分配有 Intune 许可证。 如果设备已激活并处于使用状态，则在将设备恢复出厂设置后才能应用配置文件。 如果打开注册计划托管的 iOS 设备，用户将看到以下信息：  

1. “设置 iOS 设备”- 用户可从以下选项中选择：
  - “设置为新设备”
  - “从 iCloud 备份中还原”
  - “从 iTunes 备份中还原”
2. 通知用户“Microsoft 将自动配置你的设备”。 还提供了以下其他配置详细信息：

  “配置允许 Microsoft 无线管理此设备。”

  “管理员可以帮助你设置电子邮件和网络帐户、安装和配置应用以及远程管理设置。”

  “管理员可能禁用功能、安装和删除应用、监视和限制你的 Internet 流量以及远程擦除此设备。”

  “配置由 <br>MicrosoftIntune iOS 团队<br>提供，地址 One Microsoft Way, Redmond, WA 98052 (USA)”
