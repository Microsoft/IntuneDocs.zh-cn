...
- **远程主机不可导出的凭据委派**  
  远程主机允许委派不可导出的凭据。 在使用凭据委派，设备到远程主机，与远程主机上的攻击者公开凭据被盗的风险的用户提供凭据的可导出版本。 如果启用此策略设置，主机支持“受限管理”或“远程 Credential Guard”模式。 如果禁用或未配置此策略设置，则不支持“受限管理”和“远程 Credential Guard”模式。 用户始终需要将其凭据传递给主机。  
...
- **需要密码**  
  ...
  **默认**: 是  
    ...
    当*需要使用密码*设置为*是*，有下列设置。

    - **密码最小字符集数**  
      强 PIN 或密码所需的复杂元素类型（大写和小写字母、数字和标点符号）的数量。 PIN 强制实施的桌面和移动设备的以下行为： 1-仅 2-数字和小写字母数字是否为所需的 3-小写字母、 数字和大写字母所需。 在桌面 Microsoft 帐户和域帐户中不支持。 4 - 必须为数字、小写字母、大写字母和特殊字符。 在桌面中不支持。 默认值为 1。 
  ...
  - **防止重用以前的密码**  
    指定可以在历史记录中存储的无法使用的密码数。 该值包括用户的当前密码。 例如，使用的设置*1*选择新密码时，用户不能重复使用他们当前的密码。 如果设置为*5*意味着用户不能将其新的密码设置为他们当前的密码或任何其以前的四个密码。
    
    **默认值**：24  

- **阻止幻灯片放映**  
  禁用“电脑设置”中的锁屏界面幻灯片放映设置，并阻止在锁屏界面上播放幻灯片放映。 默认情况下，用户可以启用在计算机锁定后运行的幻灯片放映。 如果启用此设置，用户不能修改幻灯片放映设置在电脑设置，可以开始没有幻灯片放映。
...
- **Internet Explorer internet 区域仅允许批准的域使用 tdc ActiveX 控件**  
  如果该用户可以运行在网站上的 TDC ActiveX 控件，此策略设置控制。 如果启用此策略设置，则 TDC ActiveX 控件不会从此区域中的网站运行。 如果禁用此策略设置，TDC ActiveX 控件会从此区域中的所有站点运行。
...
- **Internet Explorer 绕过有关不常见的文件的 smartscreen 警告**  
...
- **Internet Explorer 受限制区域登录选项**  
  ...
  - *只在 Intranet 区域自动登录*-使用此选项来查询用户的用户 Id 和其他区域中的密码。 询问用户后，可在会话的其余部分中以无提示方式这些值。 
  - *使用当前用户名称和密码自动登录*-使用此选项可尝试使用 Windows NT 质询响应 （也称为 NTLM 身份验证） 登录。 如果服务器支持 Windows NT 质询响应，在使用登录用户的网络用户名和密码登录。 如果服务器不支持 Windows NT 质询响应，则要求用户提供用户名和密码。 
...
- **Internet Explorer intranet 区域不会运行反恶意软件防范 Active X 控件**此策略设置确定 Internet Explorer 是否对 ActiveX 控件，以检查它们是否可以安全地在页面上加载运行反恶意软件程序。 如果启用此策略设置，则 Internet Explorer 不会为了确定是否可安全创建 ActiveX 控件实例而检查反恶意软件程序。 如果禁用此策略设置，则 Internet Explorer 始终都会检查反恶意软件程序，以确定是否可安全创建 ActiveX 控件实例。 如果未配置此策略设置，则 Internet Explorer 不会为了确定是否可安全创建 ActiveX 控件实例而检查反恶意软件程序。 用户可以使用“Internet Explorer 安全”设置打开或关闭此行为。
...
- **Internet Explorer internet 区域登录选项**  
  利用此策略设置，可以管理登录选项的设置。 如果启用此策略设置，则可以从下列登录选项中进行选择。 匿名登录，以禁用 HTTP 验证并只为通用 Internet 文件系统 (CIFS) 协议使用来宾帐户。 提示用户名和密码以询问用户的用户 ID 和密码。 询问用户后，可在会话的剩余阶段中自动使用这些值。 仅在 Intranet 区域中自动登录，以询问用户在其他区域中的用户 ID 和密码。 询问用户后，可在会话的其余部分中以无提示方式这些值。 用当前用户名和密码自动登录，以尝试使用 Windows NT 质询响应进行登录（也称为 NTLM 验证）。 如果服务器支持 Windows NT 质询响应，使用登录用户的网络用户名和密码登录。 如果服务器不支持 Windows NT 质询响应，则要求用户提供用户名和密码。 如果禁用此策略设置，则登录设置为“仅在 Intranet 区域中自动登录”。 如果未配置此策略设置，登录将设置为自动登录只在 Intranet 区域中。
...
- **Internet Explorer 用户添加站点**  
  阻止用户在安全区域添加或删除站点。 安全区域是一组具有相同安全级别的网站。 如果启用此策略，将禁用安全区域的站点管理设置。 （若要了解安全区域的站点管理设置，请在“Internet 选项”对话框中单击“安全”选项卡，然后单击“站点”按钮。）如果禁用或未配置此策略，则用户可以在“受信任的站点”和“受限制的站点”区域中添加或删除网站，还可以修改“本地 Intranet”区域的设置。 此策略阻止用户更改由管理员建立的安全区域的站点管理设置。 注意：“禁用安全页”策略（位于“\用户配置\管理模板\Windows 组件\Internet Explorer\Internet 控制面板”中）可将“安全”选项卡从界面上删除，它优先于此策略。 如果启用该策略，则忽略此策略。 此外，请参阅"安全区域： 仅使用计算机设置"策略。
...
- **Internet Explorer 安全区域仅使用计算机设置**  
  将安全区域信息应用于同一台计算机的所有用户。 安全区域是一组具有相同安全级别的网站。 如果启用此策略，则用户对安全区域所做的更改将应用于该计算机的所有用户。 如果禁用或未配置此策略，则同一台计算机的用户可以建立各自的安全区域设置。 使用此策略确保将安全区域设置统一地应用于同一台计算机，不会因用户的不同而产生差异。 此外，请参阅"安全区域： 不允许用户更改策略"策略。
...
- **Internet ExplorerI Internet 区域通过脚本更新状态栏**  
  使用此策略设置，可以控制是否允许脚本在区域中更新状态栏。 如果启用此策略设置，脚本可以更新状态栏。 如果禁用或未配置此策略设置，则不允许脚本更新状态栏。
...
- **Internet Explorer 用户更改策略**  
    防止用户更改安全区域设置。 安全区域是一组安全级别相同的网站。 如果启用此策略，将禁用 Internet 选项对话框中“安全”选项卡上的“自定义级别”和安全级别滑块。 如果禁用或不配置此策略，用户可以更改安全区域的设置。 此策略可防止用户更改管理员创建的安全区域设置。 注意：“禁用安全页”策略（位于\用户配置\管理模板\Windows 组件\Internet Explorer\Internet 控制面板）优先于此策略，前者从控制面板的 Internet Explorer 中删除“安全”选项卡。 如果启用该策略，则忽略此策略。 此外，请参阅"安全区域： 仅使用计算机设置"策略。
...
- **限制对命名管道和共享的匿名访问**  
  启用时，此安全设置将限制的匿名访问共享和设置的管道: (1) 可以是命名管道进行匿名访问 （2） 共享可以匿名访问
  
  **默认**: 是  
  
- **基于 NTLM SSP 的服务器的最低会话安全性**  
  此安全设置允许服务器要求对 128 位加密和/或 NTLMv2 会话安全性进行协商。 这些值取决于 LAN Manager 身份验证级别安全设置值。 选项包括： 需要 NTLMv2 会话安全： 如果不协商消息的完整性，则连接将失败。 需要 128 位加密。 如果未协商强加密（128 位），则连接将失败。
...
- **基于 NTLM SSP 的客户端的最低会话安全性**  
  ...
  - 需要 NTLMv2 会话安全： 如果 NTLMv2 协议不协商连接将失败。 
  - *要求 128 位加密*： 如果不协商强加密 （128 位），则连接将失败。
  - 需要 NTLMv2 和 128 位加密。 
...
- **智能卡移除行为**  
    此安全设置确定从智能卡读卡器中取下登录用户的智能卡时发生的情况。 选项包括：
     - 无操作。 
     - *锁定工作站*-智能卡被移除，允许用户离开工作区中，带着，其智能卡和仍保持受保护的会话时锁定工作站。
     - 强制注销 - 取下智能卡时自动注销用户。
...
- **用空白密码阻止远程登录**  
  此安全设置确定没有密码保护的本地帐户是否可用于从物理计算机控制台以外的位置登录。 如果启用，不受密码保护的本地帐户必须使用计算机的键盘登录。 
...
- **电池处于睡眠时的待机状态**  
  此策略设置管理如果使计算机处于睡眠状态时，Windows 可以使用备用状态。 如果启用或不配置此策略设置，则 Windows 使用待机状态，将计算机置于睡眠状态。 如果禁用此策略设置，则不允许使用待机状态 (S1-S3)。
...
- **系统引导启动驱动程序初始化**  
  ...
  - 好：驱动程序已签名且未被篡改。  
  - 差 - 驱动程序被标识为恶意软件。 我们建议你不要允许已知错误的驱动程序，以初始化。 
...
- **电子邮件内容执行类型**  
  此规则阻止以下文件类型从正在运行或从在 Microsoft Outlook 或 webmail （如 Gmail.com 或 Outlook.com） 中看到一封电子邮件启动： 可执行文件 （例如.exe、.dll、 或.scr） 脚本文件 （如 PowerShell.ps，VisualBasic.vbs 文件或 JavaScript.js 文件） 脚本存档文件。
  
  **默认**： 块
  
- **网络保护类型**  
  此策略可用于在 Windows Defender 攻击防护中启用或禁用网络保护（阻止/审核）。 网络保护是 Windows Defender 攻击防护的一项功能，可阻止员工使用任何应用访问 Internet 上的钓鱼邮件、攻击宿主站点和恶意内容。 这包括阻止第三方浏览器连接到危险站点。 值类型为整数。 如果启用此设置，则会启用网络保护，且员工无法将其禁用。 可通过以下选项控制其行为：阻止和审核。 如果启用此策略并使用“阻止”选项，则会阻止用户和应用连接到危险的域。 您可以看到 Windows Defender 安全中心中的此活动。 如果启用此策略并使用“审核”选项，则不会阻止用户/应用连接到危险的域。 但是，仍可在 Windows Defender 安全中心看到此活动。 如果禁用此策略，则不会阻止用户/应用连接到危险的域。 无法在 Windows Defender 安全中心看到任何网络活动。 如果未配置此策略，网络阻止默认处于禁用状态。
...
- **Defender 可能不需要的应用操作**  
  Windows Defender 防病毒中的可能不需要的应用程序 (PUA) 保护功能可以识别和阻止在网络中的终结点上下载和安装 PUA。 这些应用程序不被视为病毒、恶意软件或其他类型的威胁，但可能在终结点上执行对性能或使用产生不利影响的操作。 PUA 也指视为信誉不佳的应用程序。 典型 PUA 行为包括： 各种类型的软件捆绑 Ad 注入到 web 浏览器驱动程序和注册表优化器检测到问题，请求付款，以修复错误，但保留在终结点上，不进行任何更改或优化 （也称为"无管理防病毒"程序）。 这些应用程序可能会增加风险的您感染了恶意软件的网络，导致恶意软件感染，若要更难识别，并且可能会浪费中清理应用程序的 IT 资源。  
