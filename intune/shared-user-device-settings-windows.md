...
- **共享的 PC 模式**： 选择**启用**启用共享 PC 模式。 在此模式下，一次只能有一位用户登录设备。 第一位用户注销前，其他用户无法登录。“未配置”（默认）使 Intune 未托管此设置，并且不会推送任何策略来控制设备上的此设置。
...
- **帐户管理**：设置为“启用”后，可自动删除来宾创建的本地帐户以及 AD 和 Azure AD 中的帐户。 用户注销设备或运行系统维护时，也会删除这些帐户。 启用时，还会设置：
  - **帐户删除**： 选择中删除帐户的时间：**在存储空间阈值**，**在存储空间阈值和活动阈值**，或**后立即注销**.此外请输入：
    - **开始删除 threshold(%)**： 输入的磁盘空间百分比 (0-100)。 总磁盘/存储空间低于输入的值时，删除缓存的帐户。 不断删除帐户以回收磁盘空间。 首先删除处于非活动状态时间最长的帐户。
    - **停止删除 threshold(%)**： 输入的磁盘空间百分比 (0-100)。 总磁盘/存储空间达到输入的值时，停止删除缓存的帐户。
...
- **睡眠超时时间 （以秒为单位）**： 输入的非活动秒 (0-100) 数，才能在设备进入睡眠模式。 如果未设定秒数，设备会在 60 分钟后进入睡眠状态。
- **电脑唤醒时登录**：设置为“已启用”，要求用户在设备退出睡眠模式时使用密码登录。 选择“已禁用”，因此用户无需输入其用户名和密码。 “未配置”（默认）使 Intune 未托管此设置，并且不会推送任何策略来控制设备上的此设置。
- **维护开始时间 （以分钟为单位从午夜）**： 输入在几分钟内 (0-1440) 自动维护任务，例如 Windows 更新的运行时的时间。 默认开始时间为午夜，或零 (`0`) 分钟。 通过输入从午夜开始计算的开始时间（分钟数），更改开始时间。 例如，如果希望从凌晨 2 点开始维护，请输入 `120`。 如果希望从晚上 8 点开始维护，请输入 `1200`。
