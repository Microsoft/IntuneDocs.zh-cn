...
- **阻止无线 PKI 更新**：**阻止**用户接收软件更新，除非设备已连接到计算机。 **未配置**（默认）：允许设备在不连接到计算机的情况下接收软件更新。
...
- **设备锁定时允许使用 usb 附件**：**允许**usb 附件将数据与在一小时内被锁定的设备交换。 "**未配置**" （默认值）不会更新设备上的 USB 限制模式，如果在一小时内锁定，则将阻止 USB 附件从设备传输数据。
...
- **iTunes 限制级音乐、播客或新闻内容**：选择“阻止”可阻止 iTunes 限制级音乐、播客或新闻内容  。 “未配置”  （默认）则允许设备访问存储中认定为成人的内容。 iOS 13 和更高版本可能只需要受监督的设备。 
