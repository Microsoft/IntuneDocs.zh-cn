...
ms.locfileid: "57566618"
---
# <a name="ios-device-settings-to-use-common-ios-features-in-intune"></a>若要在 Intune 中使用 iOS 功能，常见的 iOS 设备设置
...
使用这些功能来控制 iOS 设备作为移动设备管理 (MDM) 解决方案的一部分。
...
2. 在“类型”中，选择添加“应用”或“文件夹”。
    ...
    - **添加文件夹**：选择此选项可向屏幕上的程序坞添加文件夹。 
      ...
      2. 选择“添加”，并输入以下属性：
          ...
          - **页面名称**： 输入页面的名称。 此名称可便于你在 Azure 门户中参考。 它不会显示在 iOS 设备上。
          - **应用名称**： 输入应用程序的名称。 此名称可便于你在 Azure 门户中参考。 它不会显示在 iOS 设备上。
          - **应用程序包 ID**：输入应用的捆绑 ID。 有关一些示例，请参阅（本文中的）[内置 iOS 应用的捆绑 ID](#bundle-ids-for-built-in-ios-apps)。
...
2. 输入以下属性：
    ...
    - **通知**：“启用”或“禁用”应用向设备发送通知。
       ...
       - **应用图标上的徽章**： 选择**启用**若要将一个徽章添加到应用图标。 通知提醒表示应用已发送通知。
...
2. 选择“筛选器类型”。 选项包括：
    ...
    - **配置 URL**：使用 Apple 的内置 Web 筛选器来查找成人术语，包括猥亵和露骨色情语言。 此功能在网页加载时评估每个网页，并发现和阻止不适合的内容。 还可以添加不希望筛选器检查的 URL。 或屏蔽特定 URL，无论 Apple 的筛选器设置如何。
      ...
      - **允许的 Url**:**添加**你想要允许的 Url。 这些 URL 可绕过 Apple 的 Web 筛选器。
    ...
    - **仅限特定网站**（仅适用于 Safari Web 浏览器）：向 Safari 浏览器的书签添加这些 URL。 用户只能访问这些网站；无法打开其他任何网站。 仅在知道用户可以访问的 URL 的确切列表时使用此选项。
      ...
      - **书签路径**： 输入要存储书签的路径。 例如，输入 `/Contoso/Business Apps`。 如不添加路径，则书签将被添加到设备上的默认书签文件夹中。
      - **标题**：为书签输入描述性标题。
...
- **壁纸显示位置**： 选择要显示的图像的设备上的某个位置。 选项包括：
  - **未配置**： 自定义映像不会添加到设备。 设备使用操作系统默认图像。
  - **锁定屏幕**： 将图像添加到锁定屏幕。
  - **主屏幕**： 将图像添加到主屏幕。
