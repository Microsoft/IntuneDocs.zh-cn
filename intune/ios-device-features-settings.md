...
1. 选择“设置”中的“共享设备配置(仅监督模式)”。
2. 输入以下设置：

    - **资产标记信息：** 输入有关该设备资产标记的信息。 例如，输入 `Owned by Contoso Corp`。 

      输入的文本显示在设备上的登录窗口和锁定屏幕中。

    - **锁屏脚注：** 输入一个可帮助在设备丢失或被盗时将其找回的注释。 例如，输入类似于 `If found, call Contoso at ...` 的内容。
