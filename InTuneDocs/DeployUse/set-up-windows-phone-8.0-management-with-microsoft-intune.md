...
必须先设置管理要求，然后才能使用 Intune 管理 Windows Phone 移动设备。 [创建 DNS CNAME](set-up-windows-phone-management-with-microsoft-intune.md)可帮助用户连接到 Intune 公司门户。 Windows Phone 8.0 要求使用 Symantec 证书建立设备和 Intune 之间的加密 IP 连接。 还要求使用证书对业务线应用签名。
...
1.  **设置 Intune**
    如果你尚未设置，请通过[将移动设备管理机构设置](https://technet.microsoft.com/library/mt346013.aspx)为“Microsoft Intune” 并设置 MDM，为管理移动设备做好准备。
...
2.  **为注册服务器地址设置 DNS 别名** （可选）
    ...
    3.  为公司的域创建 **CNAME** DNS 资源记录。 CNAME 资源记录必须包含以下信息：
        ...
        |主机名|指向|TTL|
        |-------------|-------------|-------|
        |enterpriseenrollment.company_domain.com|enterpriseenrollment.manage.microsoft.com |1 小时|
        ...
        -   `manage.microsoft.com` – 支持从电子邮件的域名重定向到具有域识别的 Intune 服务
...
3.  **支持应用签名的证书管理**
    [不会访问 Windows Phone 应用商店和/或需要业务线应用的 Windows Phone 8.0 和 Windows Phone 8.1 需要此管理。]
    ...
    5.  **导出签名证书**
        ...
        在“证书导出向导”中，选择“是，导出私钥”，然后单击“下一步”. “选择个人信息交换 –PKCS #12 (.PFX)”，然后选中“如果可能，在证书路径中包括所有证书”。 完成向导。 有关详细信息，请参阅[导出带私钥的证书](http://go.microsoft.com/fwlink/?LinkID=203031).
    ...
    6.  **下载公司门户应用并对其进行签名**
        ...
        1.  **下载公司门户**
            从下载中心下载 [适用于 Windows Phone 的 Intune 公司门户](http://go.microsoft.com/fwlink/?LinkId=268440) 。 默认安装位置是 `C:\Program Files (x86)\Microsoft Corporation\Windows Intune Company Portal for Windows Phone`.

        2.  **下载 Windows Phone 8.0 SDK**
            下载 [Windows Phone SDK](http://go.microsoft.com/fwlink/?LinkId=615570).

        3.  **对公司门户应用进行代码签名**
            使用与 SDK 一同下载的 XAPSignTool 应用，用从 Symantec 证书创建的 .pfx 文件对公司门户进行签名。 有关详细信息，请参阅[如何使用 XapSignTool 对 Windows Phone 的企业应用进行签名](http://go.microsoft.com/fwlink/?LinkID=280195).

    7.  **将公司门户应用上传到 Intune**
        上传已签名的公司门户应用文件和你的代码签名证书，以便最终用户可以使用此应用。

        1.  在 [Intune 管理控制台](http://manage.microsoft.com)中，单击“管理” &gt; “Windows Phone”.
...
2.  **下载 Windows Phone SDK**
    下载 [Windows Phone SDK 8.0](http://go.microsoft.com/fwlink/?LinkId=615570) (http://go.microsoft.com/fwlink/?LinkId=268439) 并将 SDK 安装到你的计算机。 需要此 SDK 来生成应用程序注册令牌。

3.  **生成 AETX 文件**
    通过使用 AETGenerator.exe（Windows Phone SDK 8.0 的一部分），从 Symantec PFX 文件生成应用程序注册令牌 (.aetx) 文件。 有关如何创建 AETX 文件的说明，请参阅 [如何为 Windows Phone 生成应用程序注册令牌](https://msdn.microsoft.com/library/windows/apps/jj735576.aspx)

4.  **下载适用于 Windows 8.1 的 Windows SDK**
    下载并安装 [Windows Phone SDK](http://go.microsoft.com/fwlink/?LinkId=613525) (http://go.microsoft.com/fwlink/?LinkId=613525)。 请注意，公司门户应用中包含的 PowerShell 脚本使用默认安装位置 `${env:ProgramFiles(x86)}\Windows Kits\8.1`。 如果安装到其他位置，则必须包括 cmdlet 参数中的位置。

5.  **使用 PowerShell 对应用进行代码签名**
    以管理员身份在主计算机上打开随 Windows SDK（Symantec 企业移动代码签名证书）一起安装的“Windows PowerShell”，导航到 Sign-WinPhoneCompanyPortal.ps1 文件并运行该脚本。
