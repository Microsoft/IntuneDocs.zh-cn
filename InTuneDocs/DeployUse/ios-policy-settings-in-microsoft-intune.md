...
|**密码中所需的复杂字符数**|此设置指定了密码中必须包括的符号字符（如**#**或**@**）数。|
...
|**允许在锁定屏幕中使用控制中心**|控制设备锁定时是否可以访问控制中心应用。|
|**允许在锁定屏幕中使用通知视图**|允许用户在不解锁设备的情况下访问通知视图。|
|**允许在锁定屏幕中使用今日视图**|控制设备锁定时是否可以查看通知。|
...
|**允许托管应用将数据同步到 iCloud**|允许你使用 Intune 管理的应用将数据同步到用户 iCloud 帐户。|
|**允许 Handoff 在另一台设备上继续活动**|Handoff 使你能够在一台 iOS 或 Mac OS X 设备上继续进行在另一台 iOS 设备上开始的工作。|
...
|**允许自动填充**|用户可以更改浏览器中的自动完成设置。|
|**允许使用弹出窗口阻止程序**|启用或禁用浏览器弹出窗口阻止程序。|
|**允许使用 Cookie**|允许设备 Web 浏览器使用 Cookie。|
|**允许使用 Java 脚本**|允许在浏览器中运行 Java 脚本。|
|**允许使用欺诈警告**|允许在设备浏览器中使用欺诈警告。|
...
|**允许在其他非托管应用中使用托管文档**|允许在任何应用中查看公司文档。<br>**示例：**你想要防止用户将文件从 OneDrive 应用保存到 Dropbox。 将此设置配置为“否”。 设备收到策略后（例如在重新启动后），将不再允许保存。|
|**允许在其他托管应用中使用非托管文档**|允许在公司托管的应用中查看任何文档。|
|**允许视频会议**|允许在设备上使用视频会议应用，如 Facetime。|
|**允许媒体存储中有成人内容**|允许设备访问存储中认定为成人的内容。|
|**允许用户从标记为“Erotica”的 iBook 商店下载内容**|允许用户下载类别为“Erotica”的书籍。|
...
|**要求提供配对密码来传出 AirPlay 请求**|可以使用 Airplay 向其他 Apple 设备流式传输内容。 使用此设置要求在连接到其他设备时提供配对密码。|
...
|设置名|详细信息|
|----------------|--------------------|
|**设备处于监督模式时允许激活锁定**|在已监督 iOS 设备上启用激活锁定。|
...
|**允许 AirDrop**|允许使用 Airdrop 功能，与附近的设备相互传输内容。|
...
|**允许擦除设备上的所有内容和设置选项**|允许用户使用擦除设备上的所有内容和设置选项。|
...
|**允许主机配对控制 iOS 设备可与之配对的设备**|主机配对使管理员能够控制 iOS 7 设备可与之配对的设备。|
