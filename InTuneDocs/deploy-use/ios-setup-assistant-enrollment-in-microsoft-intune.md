...
> [!Important]
> 目前，Intune 可以指定一个“默认”设备注册配置文件，这意味着当新序列号与 Apple DEP 服务同步时，新的序列号将自动分配给该默认配置文件。 之后，当租户迁移到新的 Azure 门户时，将不再能设置默认配置文件，也不再能为该配置文件自动分配序列号。 而必须手动为配置文件分配序列号。 [了解详细信息](https://docs.microsoft.com/intune-azure/enroll-devices/enroll-ios-devices-using-device-enrollment-program)
