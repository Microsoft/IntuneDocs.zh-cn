...
# <a name="microsoft-intune-app-sdk-for-ios-developer-guide"></a>Microsoft Intune App SDK for iOS 开发人员指南
...
Microsoft Intune App SDK for iOS 支持将 Intune 应用保护策略集成到 iOS 应用中。 启用了 MAM 的应用程序是指与 Intune App SDK 集成的应用程序，允许 IT 管理员在通过 Intune 移动应用程序管理 (MAM) 主动管理移动应用时在该应用上部署策略。
...
* 查看 [Intune App SDK for iOS 许可条款](https://github.com/msintuneappsdk/ms-intune-app-sdk-ios/blob/master/Microsoft%20License%20Terms%20Intune%20App%20SDK%20for%20iOS%20.pdf)。 打印并保留一份许可条款，以留作记录。 下载和使用 Intune App SDK for iOS 即表示同意这些许可条款。  如果不接受这些条款，请不要使用此软件。
...
## <a name="building-the-sdk-into-your-mobile-app"></a>将 SDK 构建到移动应用中
...
Intune App SDK 在身份验证和条件启动时使用 ADAL。 它还依赖于 ADAL 以注册带有 MAM 服务的用户标识，用于不含设备注册方案的管理。

通常情况下，ADAL 要求应用注册 [Azure Active Directory] 并获得唯一的 ID（称为 ClientID）和其他标识符，从而确保授予该应用的令牌的安全。 连接 Azure Active Directory 时，Intune App SDK 使用默认的注册值。  
...
## <a name="register-your-app-with-intune-mam-service"></a>使用 Intune MAM 服务注册应用
...
### <a name="de-register-accounts"></a>取消注册帐户
...
## <a name="enrolling-without-prior-sign-in"></a>未登录注册
...
## <a name="implementing-save-as-controls"></a>实施“另存为”控件
...
**isSaveToAllowedForLocation** API 提供常量，以检查 IT 管理员是否允许将数据保存到以下位置：
...
## <a name="configure-intune-app-sdk-settings"></a>配置 Intune App SDK 设置
...
## <a name="recommended-ios-best-practices"></a>建议使用的 iOS 最佳做法
...
应在用户注销应用程序之前调用此方法。  作为注销的一部分，如果应用程序已删除用户的数据，则可将 `doWipe` 设置为 false。  但是，如果应用程序实际未删除用户的数据，则该标志应设置为 true，以便 SDK 可以手动删除数据本身。

**是否有其他可以取消注册应用程序的方法？**
是，IT 管理员可以向该应用程序发送选择性擦除命令，撤销用户注册、取消用户注册，并擦除用户的数据。  SDK 自动处理此方案，并将通过取消注册委托方法发送通知。

## <a name="submitting-your-app-to-the-app-store"></a>将应用提交到应用商店
