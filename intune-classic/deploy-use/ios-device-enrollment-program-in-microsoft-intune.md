...
- 组织必须加入 Apple DEP 并让设备通过该计划。 该过程的详细信息，可以通过以下网站获得：  [https://deploy.apple.com](https://deploy.apple.com)进行管理。此计划的优势在于无需动手设置设备，不要使用 USB 线缆将每台设备与计算机相连。

- 可以通过 DEP 注册公司拥有的 iOS 设备之前，需要从 Apple 获得 DEP 令牌。 使用此令牌，Intune 可以同步企业拥有的已加入 DEP 的设备的相关信息。 它也允许 Intune 将注册配置文件上传至 Apple，并向设备分配这些配置文件。

## <a name="steps-to-enroll-ios-devices-by-using-apple-dep-management"></a>使用 Apple DEP 管理注册 iOS 设备的步骤

下列步骤介绍了如何使用 Apple DEP 管理即时注册 iOS 设备。 随着组织添加和删除设备，你可能需要重复执行下面的一些步骤，如添加或删除序列号，如下所述。
...
3.  输入“MDM 服务器名称”，然后选择“下一步”。 服务器名称供参考，用于识别移动设备管理 (MDM) 服务器。 并不是 Microsoft Intune 服务器的名称或 URL。

4.  此时，“添加 &lt;服务器名称&gt;”对话框打开。 选择“选择文件…” 以上传 .pem 文件，然后选择“下一步”。

5.  “添加 &lt;ServerName&gt;”对话框显示“你的服务器令牌”链接。 将服务器令牌 (.p7m) 文件下载到计算机中，然后选择“完成”。
...
2. 选择“上传 DEP 令牌”。 **转到**证书 (.p7m) 文件，输入你的 **Apple ID**，然后选择“上载”。
...
2. 提供**常规**详细信息，包括**名称**和**说明**，然后指定分配到配置文件的设备是否拥有用户关联或是否属于某个组：
   ...
   - **用户关联提示**：必须在初始设置过程中将设备与用户相关联，然后才能以相应用户的身份访问公司数据和电子邮件。 应为属于用户且需要使用公司门户（即需要安装应用）的受 DEP 管理设备设置“用户关联”。 在具有用户关联的 DEP 设备上注册期间，多重身份验证 (MFA) 不起作用。 注册后，MFA 可以在这些设备上如预期一样正常运作。 注册 DEP 设备时，需要在首次登录时更改密码的新用户不会获得提示。 此外，在 DEP 注册过程中，密码已过期的用户不会获得重置密码的提示，必须使用其他设备重置密码。
...
2. 选择要向其分配创建的配置文件的设备。 可以选择“所有设备”或特定设备，然后选择“添加”。
...
2. 若要在同步后查看 DEP 托管的设备，在 [Microsoft Intune 管理控制台](https://manage.microsoft.com)中，转到“组”&gt;“所有设备”&gt;“企业预注册设备”&gt;“按 iOS 序列号”。 在“按 iOS 序列号”工作区中，在打开设备并运行设置助理以注册设备之前，托管设备的“状态”将一直显示为“未连接”。
   ...
   - 每七天只运行一次完全的 DEP 同步。 无论之前是否同步了序列号，在完全同步时，Intune 都将刷新 Apple 分配给 Intune 的每个序列号。 如果距离上一次完全同步不到七天就尝试运行完全同步，Intune 只会刷新 Intune 中尚未列出的序列号。
