...
* 将 VPP 令牌导入 Intune 之后，不要将同一令牌导入任何其他设备管理解决方案。 这样做可能会丢失许可证分配和用户记录。
* 在开始将 iOS VPP 与 Intune 配合使用之前，先删除使用其他移动设备管理 (MDM) 供应商创建的任何现有 VPP 用户帐户。 为保障安全，Intune 不会将这些用户帐户同步到 Intune。 Intune 将仅同步 Apple VPP 服务中由 Intune 创建的数据。
